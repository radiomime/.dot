...
custom_config = hs.fs.pathToAbsolute(
  os.getenv("HOME") .. "/.config/hammerspoon/private/config.lua"
)
...
if string.len(hstype_keys[2]) > 0 then
  spoon.ModalMgr.supervisor:bind(
    ...
    "Type Browser Link",
    function()
      local safari_running =
        hs.application.applicationsForBundleID("com.apple.Safari")
      local chrome_running =
        hs.application.applicationsForBundleID("com.google.Chrome")
