...
local function r_inspect_settings(structure, limit, separator)
    ...
    if ts == "table" then
        for k, v in pairs(structure) do
            ...
            limit = r_inspect_settings(
                v,
                limit,
                separator .. "." .. tostring(k)
            )
            if limit < 0 then
                break
            end
    ...
    if type(structure) == "function" then
        ...
        io.write("-- neo", separator:sub(2), " = function ()\n")
    else
        io.write("neo", separator:sub(2), " = ", tostring(structure), "\n")
    end
    return limit - 1
end
...
function utils.generate_settings()
    ...
    -- write all `neo` related settings to `lv-settings.lua` file
    r_inspect_settings(neo, 10000, ".")
...
function utils.reload_lv_config()
    ...
    vim.cmd("source ~/.local/share/nvim/neo/lua/settings.lua")
    vim.cmd("source " .. USER_CONFIG_PATH)
    require("keymappings").setup() -- this should be done before loading the plugins
    vim.cmd("source ~/.local/share/nvim/neo/lua/plugins.lua")
