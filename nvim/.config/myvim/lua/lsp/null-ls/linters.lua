...
function M.setup(filetype, options)
  if not neo.lang[filetype] or (linters_by_ft[filetype] and not options.force_reload) then
    return
  end

  linters_by_ft[filetype] = M.list_configured(neo.lang[filetype].linters)
